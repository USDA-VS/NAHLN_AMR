...
# AMR Nextflow Pipeline - NCAH SciComp Apptainer Wrapper
# USAGE:
...
#
# EXAMPLE:
#   sbatch /path/to/cluster_amr_nextflow_scicomp_apptainer.sh \
#       --input_dir /path/to/my/data \
#       --outdir /path/to/results
#
#   # With custom MLST lookup file
#   sbatch /path/to/cluster_amr_nextflow_scicomp_apptainer.sh \
#       --input_dir /path/to/my/data \
#       --outdir /path/to/results \
...
#   - Uses Apptainer containers via ncah_apptainer profile
...
if [ -z "$INPUT_DIR" ] || [ -z "$OUTPUT_DIR" ]; then
    ...
    echo "Usage: sbatch $0 --input_dir /path/to/data --outdir /path/to/results [--mlst_lookup_json /path/to/lookup.json]"
    exit 1
fi
...
echo ""
echo "Starting Nextflow run with Apptainer..."
