...
class Kraken2_Identification:
    ...
    def __init__(self, **kwargs):
        ...
        # self.db = "/project/bioinformatic_databases/databases/kraken/k2_eupathdb48_20201113" #Eukaryotic - external.  Database seems to only have single cell represenations.  Not helpful
        ...
        if db_contents:
            with open(f'{self.kraken_db}/README', 'r') as opened_file:
                for line in opened_file:
                    print(f'{line.strip()}')
            print(f'\nDatabase location: {self.kraken_db}\n')
    ...
    def kraken2_run(self,):
        ...
        if len(FASTQ_list) == 2:
            os.system(f'kraken2 --db {kraken_db} --threads {threads} --paired {FASTQ_list[0]} {FASTQ_list[1]} --output {sample_name}-outputkraken.txt --report {sample_name}-reportkraken.txt')
        elif len(FASTQ_list) == 1:
            os.system(f'kraken2 --db {kraken_db} --threads {threads} {FASTQ_list[0]} --output {sample_name}-outputkraken.txt --report {sample_name}-reportkraken.txt')
        elif FASTA:
            os.system(f'kraken2 --db {kraken_db} --threads {threads} {FASTA} --output {sample_name}-outputkraken.txt --report {sample_name}-reportkraken.txt')
        else:
...
if __name__ == "__main__": # execute if directly access by the interpreter
    ...
    kraken2 = Kraken2_Identification(FASTA=args.FASTA, FASTQ_R1=args.FASTQ_R1, FASTQ_R2=args.FASTQ_R2, directory=args.directory, db_contents=args.db_contents)
