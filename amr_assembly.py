...
from amr_fastq_stats_seqkit import FASTQ_Stats
...
class Assemble(Setup):
    ...
    def run(self,):
        ...
        if len(FASTQ_list) == 2:
            ...
            print(f'Seeing 1 Fastq option - trying long read assembly with Unicycler')
           # subprocess.run(["spades.py", "-s", FASTQ_list[0], "-o", "spades_assembly"], capture_output=True)
            subprocess.run(["unicycler", "-l", FASTQ_list[0], "-o", "unicycler_assembly"], capture_output=True)
        else:
        ...
        elif os.path.exists(f'{cwd}/unicycler_assembly/assembly.fasta'):
            shutil.copy2(f'{cwd}/unicycler_assembly/assembly.fasta', f'{cwd}/{sample_name}.fasta')
            self.FASTA = f'{cwd}/{sample_name}.fasta'
            if not debug:
                shutil.rmtree(f'{cwd}/unicycler_assembly')
        else:
    ...
    def stats(self, FASTA=None):
        ...
        elif self.FASTQ_R1 and self.FASTQ_R2 == None:
            self.coverage_title = 'FASTQ calculated mean coverage' #read count * read size / total assembly length
            mean_coverage = ((int(self.R1.num_seqs.replace(',', '')) * float(self.R1.avg_len.replace(',', ''))))/total_contig_lengths
        else:
